...
Stochastic population-based simulators play an important role in many industrial and scientific settings. Many problems surrounding such simulators involve solving the “inverse problem”, which consists of finding the simulator parameters that generate output that is closest to a given set of field observations.
The framework of probabilistic programming naturally allows to express and solve such inverse problems. Unfortunately, application of existing tools requires refactoring of existing simulators within specialised probabilistic programming frameworks. This is usually infeasible given the size and complexity of existing simulators. Recent work by Baydin et al now enables existing simulators to be treated as a probabilistic program without extensive modification. We, in this paper, extend these exciting developments in two ways: First, we develop a mathematical formalism that allows us to succinctly define posterior densities of programs generated from stochastic population-based simulators. We then illustrate the applicability of Baydin et al’s approach by hijacking a series of important stochastic population-based Malaria epidemiology models. Our approach is capable of finding suitable model parameters much more efficiently than currently used approaches in the literature and refines . 
...
\label{fig:hijacking} 
\end{wrapfigure}
One such machine learning paradigm that has the ability to resolve these issues is \emph{Probabilistic programming}\cite{gordon2014probabilistic,le2016inference}. 
...
the automation of complex statistical inference procedures\cite{zhou2019lf,goodman2012church,wood2014new}, which enables us to efficiently calibrate
model parameters, and we can also track how the internals of the program change during
run time, enabling a full analysis of the program structure and traces generated, which provides greater interpretability, see Figure~\ref{fig:hijacking}.
...
\paragraph{Related work} Our method of hijacking simulators builds on the work of~\cite{baydin2018efficient}. 
...
between populations, that are utilized for modeling epidemics, multi-agent systems and financial economics  \bg{add citations}. 
...
simulator output. We provide a theoretical justification for our approach and many examples.
