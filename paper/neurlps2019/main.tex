...
each individual system, such as specifying a model in the given language, means that it can be difficult to re-implement the myriad of scientific and industrial simulators within an existing PPSs, something that is not typically feasible, nor practical.  
However, recent work
by \cite{baydin2018efficient} demonstrated a pathway for how to turn a particular
type of event-based simulators into a probabilistic program. 
...
Probabilistic programming\cite{gordon2014probabilistic} is paradigm that enables users to write, and perform inference in, complex probabilistic models in a programmatic, yet efficient and simplified way. 
The source code of the given model, or equivalently \emph{program}, is then transformed into a program density, representing the joint density of the model.
By having direct access to the source code we can perform interesting 
operations within a probabilistic programming system~(PPS),
such as analysing the model code, which enables statistically correct inference \cite{gram2018hamiltonian} in an automated way \bg{Add citations - black box VI etc} to be performed. 
In addition to this, as models can be directly expressed in program code, we can encode other structures make us of the full structure of a given language.
