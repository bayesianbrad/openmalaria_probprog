...
\begin{equation}
  ...
  \end{cases}
\end{equation}
where $\mathcal{B}(\cdot)$ is satisfied by the simulator and $f_a_i(\cdot)$ represents a
...
\begin{equation}
  ...
  \begin{cases}
  \prod_{i=1}^{n_x} f_{a_i}(x_i | \eta) \prod_{j=1}^{n_y} g_{b_j}(y_j | \phi) \texttt{if} \mathbal{B(x_{1:n_x})} = 1\\
  0
  \end{cases}
\end{equation}
...
\begin{equation}
  w( x_{1:n_x}) = \prod^{n_x}_{i=1}\frac{f_a_i(x_i | \eta_i)}{q_i(x_i |f_a_i, \eta_i)} \prod^{n_y}_{j=1} g_b_j(y_j | \phi_j)
\end{equation}
...
\begin{equation}
  w_j(x1:K(j)) = g_b_j(y_j | \psi_j) \prod^{K(j)}_{K(j-1)+1} v_i
\end{equation}
where we define $v_j = \frac{f_a_j(x_j | \eta_j)}{q_j(x_j |f_a_j, \eta_j)}$ \textbf{Note: Ask Tom why the proposal is also dependent
on $f_a_i$. }
        ...
        \Procedure{SMC}{$T,M$,\textbf{x}, \textbf{w},\textbf{v}, \textbf{g}}
          \For{$t = 1:T$}
            \For{$m = 1:M$}
              \State $w^{m} = 1  $
              \For{$k = K(t-1)+1:K(t)$}
                \State $x^m_k \sim q_k(x^m_k) f^m_{a_k,\eta^m_{j}}$
                \State $w^m_t \gets w^t_m v_j^m(x_k^m)$
                \EndFor
            \EndFor
          \State $w^m_t \gets w^m_t g_bt^m(y_t^m | \psi^m_t)$
...
as we do not have a functional form for all parts of the simulator \textbf{Note: Re Word}
...
\begin{figure}[ht!]
  \centering
  \label{fig:parasite}
  \includegraphics[width=0.5\textwidth]{parasite_model.pdf}
...
\begin{figure}[ht!]
  ...
  the distributions of the rejection sampling statements. }
\end{figure}
