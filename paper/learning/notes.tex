...
We do this by  Replacing the rejection sampling loops with 
...
\end{figure}
However, in order todo this, we must correctly specify our variational objective.
...
Let $f(\epsilon)$ be a differentiable transform of noise $\epsilon \sim \pi(\epsilon)$, which models 
some process to generate some data $y = f(\epsilon)$. 
Then the probability density of $y$ under the flow is
obtained by a change of variables: 
\begin{equation}
p(y) = \pi(f^{-1}(y)) \left|\det \frac{\partial f^{-1}}{\partial y}\right|
\end{equation}

where the $y$'s are generated from the rejection sampling loops. 

\begin{itemize}
  \item Do we need to know the base distribution that the RS block is sampling from? i.e there maybe be multiple $\pi(\epsilon)$ in each 
  RS block. 
  \item Is it right to say here that $y$ is the output of the RS block, we do not know $f$ here. 
  \item We need to learn $f(.)$
\end{itemize}
