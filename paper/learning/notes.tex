...
and hence measures the cumulative parasite load, and  \textbf{TODO: To finish}
  % \begin{align}
%   \mu_i &= N \\ % pop size
%   % disease dynamics
...
\begin{align}
  J(\eta) = \mathbb{E}_{p(x)}[KL(p(x) || q(x; \eta))] \\
   = \int p(x') [\int p(x)\ln\left(\frac{p(x)}{q(x;\eta)}\right) dx]dx' \\
  %  \int p(x')dx' \gets 1 \\
