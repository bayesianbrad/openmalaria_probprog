...
Rejection samplers allow from sampling from densities $p(X=x)$ by constructing an upper bound to $p(X=x)$
specified by some predicate expression that determines whether we accept or reject samples from a density proportional to the bounding envelope. 
The functions we choose to define the envelope are much simpler than $p(X=x)$ and easy to sample from. 
When designing rejection samplers we aim to reduce the rate of rejections and increase the rate of acceptance, for computational purposes.
However, designing efficient and expressive rejection samplers is challenging, leading to complex and inefficient constructions of 
rejection samplers in simulators, see appendix~\ref{app:exampleprograms} for examples.
Due to these inefficiencies, and additional complications that arise in nested structures, as described in Section~\ref{sec:nested}
we propose a scheme to amortize such processes and learn accurate surragotes for the individual sub-programs, which in many cases are complex rejection
sampling schemes.


% are heavily used when designing simulators,
%  due to their ease of use and ability to model 
...
\subsection{Types of rejection samplers}

There are essentially two different forms of rejection samplers found with in simulators and program code. 
