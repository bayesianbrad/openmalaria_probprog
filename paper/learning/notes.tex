...
\subsection{Defining the Density and Sub-Densities within Simulators}
...
For $R1$ we have one input and one output, for $R2$ we have 2 inputs and 1 output. 
For the loss function we use a simple mean squared error loss and use a the regressor the learn 
and unknown mean $\mu$ and variance $\sigma^{2}$. This means that we are trying to fit a simple Normal distribution
to each rejection sampling block. Once learned we can then efficiently sample from the parameterized normal distribution, which 
represents our learned proposal $q(z_i, \ldots, z_{n} | \eta)$. 
\paragraph{Training scheme} For each rejection sampling block we learn a proposal $q(z_1, \ldots, z_i | \eta)$, in this case 
\textbf{Note: Add results here. Code almost complete}

\subsection{2nd Baseline}
Instead of using a single guassain, use a proposal that is a mixture of gaussians. 
\texbf{TODO: Add math tomorrow}
...
Flows availabile are MVP and Neural Spline flows [add citations]. 

for other techniques ask Guens. 
