...
\end{tabular}};
 ...
 \begin{tabular}{l}
  ...
  \parbox{4cm}{P(\mathcal{T}= $x_{1:n_x}| \lambda)$}
 \end{tabular}
 };
 ...
 \begin{tabular}{l}
  ...
  \parbox{4cm}{$\Omega = \{P(x_{1:n_{x}}), w\}$}
 \end{tabular}
 };
 ...
 (PyProb)       	edge [bend left=20] node[anchor=south,above]{$g_{b_{i}}(y_i, \phi_i)$} (EPC)
 (PyProb)       edge [bend left=9] node[anchor=west, above]{$x_{i}$ \emph{returned}} (OBSERVE)
 (System)       	edge [bend right=10]                                          (EPC)
 (EPC)              edge                                                    (OUTPUT)
 (System)  	edge   node[anchor=north,above]{\emph{Control}} (OBSERVE);
